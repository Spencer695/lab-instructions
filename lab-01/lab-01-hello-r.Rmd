...
It has 1846 Rows and 3 Columns. The variables are data set, x, and y.
...
Made the dinosaur graph and got calculation.
...
The star data set is very similar to the dino data set where both values are close to zero meaning little to no linear relationship. THe star data set is slightly negative while the dino is slightly positive. 
...
Very similar but negative slightly and both data sets show nearly the same linear relationship.
...
ggplot(circle_data, aes(x = x, y = y)) +
  geom_point() +
  labs(title = "circle Dataset: y vs x")
```
...
The circle data set is very similar to the dino data set with the difference in the value. They are both close to zero and show little to no linear relationship. 

- What do you notice about the correlation coefficients for dino, star, and circle?

All three are very close to zero and are nearly the same even if the data sets have different images produced. This means the the correlation does not show the true structure of the data set and shows the importance of showing a visual of it. 
